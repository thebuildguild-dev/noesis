...
function VerificationReport({ result }) {
  ...
  return (
    ...
    >
      <div className="mb-3">
        <p className="font-marker text-xs font-bold text-ink/40 uppercase tracking-wide mb-1">
          Image Analysis
        </p>
...
export default function ProofUploadModal({ habitId, habitTitle, onClose, onApproved }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ink/30 backdrop-blur-sm">
      <div
        className="bg-paper border-2 border-ink/20 w-full max-w-md shadow-lg overflow-y-auto max-h-[90vh]"
        style={{ borderRadius: radius.wobblyCard }}
      >
        {/* Header */}
        <div className="flex items-center justify-between p-4 border-b border-dashed border-muted/60">
          <div>
            <h2 className="font-marker text-lg font-bold text-ink">Proof Verification</h2>
            <p className="font-hand text-xs text-ink/40 mt-0.5 truncate max-w-[260px]">
              {habitTitle}
            </p>
          </div>
          <button
          ...
          {result ? (
            ...
            <div className="flex flex-col items-center py-8 gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="font-hand text-sm text-ink/70">AI analyzing proof…</span>
              </div>
              <p className="font-hand text-xs text-ink/40 text-center">
            ...
            <>
              {preview ? (
                <div className="mb-4">
                  <img
                    src={preview}
                    alt="Selected"
                    className="w-full h-48 object-cover border-2 border-ink/20"
                    style={{ borderRadius: radius.wobblyCard }}
                  />
                  <p className="font-hand text-xs text-ink/40 mt-1.5 truncate">{file?.name}</p>
                </div>
              ) : (
                <button
                  onClick={() => inputRef.current?.click()}
                  className="w-full h-40 flex flex-col items-center justify-center gap-3 border-2 border-dashed border-ink/20 hover:border-ink/40 hover:bg-ink/[0.02] transition-colors mb-4"
                  style={{ borderRadius: radius.wobblyCard }}
                >
                  <Camera size={32} strokeWidth={1.5} className="text-ink/30" />
                  <p className="font-hand text-sm text-ink/50">Click to upload proof image</p>
                  <p className="font-hand text-xs text-ink/30">JPG, PNG, WebP — max 10 MB</p>
                </button>
              )}
              ...
              <div className="flex gap-2">
                {preview && (
                  <button
                    onClick={handleRetry}
                    className="px-3 py-2 font-hand text-sm text-ink/50 border border-muted hover:text-ink transition-colors"
                    style={{ borderRadius: radius.btn }}
                  >
                    Change
                  </button>
